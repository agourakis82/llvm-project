...
static SDValue tryFoldMul24ToMad64_32(SDNode *N, SelectionDAG &DAG,
  ...
  // MUL_[IU]24 only reads the low 24 bits of its operands, but the mad reads
  // all 32. Fold only when both factors are known to fit in 24 bits, so the mad
  // computes the same product. This holds for mul24 nodes performMulCombine
  // formed under isU24/isI24, but not for a raw amdgcn.mul.u24 intrinsic.
  unsigned NumBits = Signed ? AMDGPUTargetLowering::numBitsSigned(X, DAG)
  ...
  if (NumBits > 24 || NumBitsY > 24)
    return SDValue();

  // Mirror tryFoldToMad64_32. On targets without full-rate 64-bit ops, avoid
  // duplicating the multiply and fold only when it feeds nothing but this add.
  ...
  SDLoc SL(N);
  // mad_[iu]64_[iu]32 takes 32-bit factors. The mul24 operands are already i32.
