  ...
  for (Constant *C : ConstantElements) {
    ...
    if (Bits == LaneBits) {
      ...
      for (unsigned Idx : seq<unsigned>(NumLanes)) {
        ...
        Constant *Lane = createEquivalentConstantWithType(Piece, LaneTy);
        if (Lane == nullptr)
          return nullptr;
        ConstantLanes.push_back(Lane);
      }
    } else {
      // C is narrower than LaneTy, or its size is not a multiple of LaneTy, so
      // it cannot be packed into LaneTy elements.
      return nullptr;
    }
  }
